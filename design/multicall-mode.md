...
other GasKillerSDK contracts it calls, so a single bundle (one BLS verification) can
...
**SDK side** (`src/GasKillerSDK.sol`): `verifyAndUpdate` verifies one BLS quorum
signature over `sha256(abi.encode(transitionIndex, address(this), targetFunction,
storageUpdates))` (GasKillerSDK.sol:68), then `StateChangeHandlerLib._runStateUpdates`
...
|---|---|---|
| **ForwardedApply** (chosen) | Callee sub-payload embedded as an ordinary `CALL` op to a new `applyForwardedUpdates` entrypoint; per-contract trusted-forwarder allowlist; one BLS check per bundle | Smallest delta, wire format unchanged, best transitionIndex coherence; trust bottoms out in allowlist policy |
...
number. No second pairing check, no new wire format, no new signing scheme.
...
struct GasKillerSDKStorage {
    ...
    IBLSSignatureChecker blsSignatureChecker; // base + 2
    uint256 blockStaleMeasure;                // base + 3
    mapping(address => bool) trustedForwarders; // NEW, base + 4
}
```

Base slot `0x321e...d000` unchanged; never reorder fields.
...
One signature, one `checkSignatures` pairing per bundle. Operators independently
...
is its own post-BLS `_stateChangeHandler`*. That is a property of A's **code**, fixed
...
3. **`require(quorumNumbers.length > 0)`** in `verifyAndUpdate` — cheap hardening; an
   empty quorum list currently skips the threshold loop entirely.
4. **Preimage v2 (future, breaking):** add `block.chainid` + a domain-separation tag to
   ...
   deployments on another chain where the same operators hold ≥66% are replayable.
   Multi-call inherits this single-mode gap; fix in a coordinated v2.
5. **Paranoid fallback, zero new code:** a callee that refuses to allowlist anyone can
   ...
   ordinary CALL op (costs one extra pairing, needs a per-callee signature from the
...
| R2 | Front-run grief: bump any callee's counter before the bundle lands → router pays BLS gas for a revert | Same class as single mode; router serialization + re-sign loop |
...
a second independent bundle costs a full pairing + registry reads + 21k base (modeled
...
1. **Done (this repo):** storage field + `applyForwardedUpdates` + denylist + interface +
   `quorumNumbers.length` hardening; mock-BLS test harness and A→B / A→B→C / repeated-B
   integration tests in `test/GasKillerForwarding.t.sol`.
