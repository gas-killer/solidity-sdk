...
from deploy_sepolia import Rpc, Plan, Ledger, expected_codehash, CHUNK, DEFAULT_BLOBS, blob_paths  # noqa: E402
...
def main():
    ...
    ap.add_argument("--blobs", default=DEFAULT_BLOBS,
                    help=f"comma-separated blob files under --artifacts, in chunk order (default: {DEFAULT_BLOBS})")
    ...
    ap.add_argument("--engine", help="engine address; enables the checkArtifacts call")
    ap.add_argument("--engine-kind", choices=["qwen", "fly"], default="qwen",
                    help="qwen: Qwen3Engine.checkArtifacts(address,bytes32,bytes32[3]); "
                         "fly: FlyEngine.checkArtifacts(address graphRoot,address warmRoot,bytes32[3])")
    ap.add_argument("--warm-root", help="fly only: the warm directory root passed as the second argument")
    ...
    plan = Plan(blob_paths(args.artifacts, args.blobs))
    ...
    cfg_words = "".join(c[2:].rjust(64, "0") for c in cfg)
    if args.engine_kind == "fly":
        if not args.warm_root:
            sys.exit("--engine-kind fly needs --warm-root (the warm directory root)")
        # calldata: selector || graphRoot || warmRoot || cfg[0..2]
        selector = "0x" + _kec(b"checkArtifacts(address,address,bytes32[3])")[:4].hex()
        data = (selector
                + addrs["root"][2:].rjust(64, "0")
                + args.warm_root[2:].rjust(64, "0")
                + cfg_words)
    else:
