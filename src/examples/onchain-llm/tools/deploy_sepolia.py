...
DEFAULT_BLOBS = "weights.bin,tokenizer.bin"   # the Qwen3 plan; keep the default so its 24,364-chunk ledger stays reproducible


def blob_paths(artifacts_dir, csv):
    """`--blobs a.bin,b.bin` -> [artifacts/a.bin, artifacts/b.bin] (order = chunk order)."""
    names = [s.strip() for s in csv.split(",") if s.strip()]
    if not names:
        raise SystemExit("--blobs must name at least one file")
    return [os.path.join(artifacts_dir, n) for n in names]


class Plan:
    """Chunk table over one or more blobs, chunked in order; every blob restarts at its own byte 0.
    Keys: c0..c<N-1> (global chunk index), then p0.., then root.

    Plan(["weights.bin", "tokenizer.bin"]) is byte-identical to the historical two-file plan
    (chunk i < n_w is weights, the rest tokenizer); Plan(w, t) still works."""

    def __init__(self, *paths):
        if len(paths) == 1 and isinstance(paths[0], (list, tuple)):
            paths = tuple(paths[0])
        if not paths:
            raise ValueError("Plan needs at least one blob")
        self.paths = list(paths)
        self.files = [open(p, "rb") for p in self.paths]
        self.lens = [os.path.getsize(p) for p in self.paths]
        self.n_per = [(n + CHUNK - 1) // CHUNK for n in self.lens]
        self.starts = [0]
        for n in self.n_per:
            self.starts.append(self.starts[-1] + n)
        self.n_chunks = self.starts[-1]

    def blob_of(self, i: int) -> int:
        return bisect_right(self.starts, i) - 1

    def payload(self, i: int) -> bytes:
        if not 0 <= i < self.n_chunks:
            raise IndexError(f"chunk {i} outside plan of {self.n_chunks}")
        k = self.blob_of(i)
        base = (i - self.starts[k]) * CHUNK
        f = self.files[k]
        f.seek(base)
        return f.read(min(CHUNK, self.lens[k] - base))

    # ---- back-compat names for the two-blob Qwen plan ----
    @property
    def w_len(self): return self.lens[0]
    @property
    def t_len(self): return self.lens[1] if len(self.lens) > 1 else 0
    @property
    def n_w(self): return self.n_per[0]
    @property
    def n_t(self): return self.n_per[1] if len(self.n_per) > 1 else 0
...
class Deployer:
    def __init__(self, args):
        ...
        self.plan = Plan(blob_paths(args.artifacts, args.blobs))
    ...
    def run(self):
        n_full = sum(max(n - 1, 0) for n in self.plan.n_per)
        est_gas = n_full * 5_364_000 + 6_000_000 * len(self.plan.n_per) + self.n_pages() * 5_400_000
        per_blob = " + ".join(f"{os.path.basename(p)} {n}" for p, n in zip(self.plan.paths, self.plan.n_per))
        print(f"[plan] sender {self.sender} | chunks {self.plan.n_chunks} "
              f"({per_blob}) | pages {self.n_pages()} | "
              f"est gas {est_gas/1e9:.2f}B", flush=True)
        print("[plan] " + ", ".join(f"{os.path.basename(p)} {n} B" for p, n in zip(self.plan.paths, self.plan.lens)), flush=True)
...
def main():
    ...
    ap.add_argument("--blobs", default=DEFAULT_BLOBS,
                    help="comma-separated blob files under --artifacts, chunked in this order "
                         f"(default: {DEFAULT_BLOBS}; fly graph: ptr.bin,edges.bin,meta.bin; fly warm: warm.bin)")
