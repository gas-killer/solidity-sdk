...
/// @dev BLS signature checker stand-in: reports a configurable signed/total stake for
/// every requested quorum so `verifyAndUpdate` can be exercised end-to-end.
contract MockBLSSignatureChecker {
    uint96 public signedStake = 100;
    uint96 public totalStake = 100;

    function setStakes(uint96 _signedStake, uint96 _totalStake) external {
    ...
    function checkSignatures(
        ...
        uint96[] memory signedStakeForQuorum = new uint96[](quorumNumbers.length);
        uint96[] memory totalStakeForQuorum = new uint96[](quorumNumbers.length);
        for (uint256 i = 0; i < quorumNumbers.length; i++) {
            signedStakeForQuorum[i] = signedStake;
            totalStakeForQuorum[i] = totalStake;
        }
        return (IBLSSignatureCheckerTypes.QuorumStakeTotals(signedStakeForQuorum, totalStakeForQuorum), bytes32(0));
    }
}
...
contract GasKillerForwardingTest is Test {
    ...
    // ERC-7201 base slot of GasKillerSDKStorage
    bytes32 internal constant SDK_CONFIG_SLOT = 0x321ebf629ed2e1e368f0890e8fdd95cf9a2ae5961b66a1805f0b2ec84e21d000;

    GasKillerSDKExposed public a;
    GasKillerSDKExposed public b;
    GasKillerSDKExposed public c;
    MockBLSSignatureChecker public blsChecker;

    function setUp() public {
        blsChecker = new MockBLSSignatureChecker();
        a = new GasKillerSDKExposed(makeAddr("AVS"), address(blsChecker));
        b = new GasKillerSDKExposed(makeAddr("AVS"), address(blsChecker));
        c = new GasKillerSDKExposed(makeAddr("AVS"), address(blsChecker));
    ...
    // ============ verifyAndUpdate end-to-end (mock BLS checker) ============

    function _emptyNonSignerStakesAndSignature()
        internal
        pure
        returns (IBLSSignatureCheckerTypes.NonSignerStakesAndSignature memory params)
    {}
    ...
    function test_verifyAndUpdate_emptyQuorumNumbersReverts() public {
        vm.roll(100);
        ...
        vm.expectRevert(IGasKillerSDK.EmptyQuorumNumbers.selector);
        a.verifyAndUpdate(msgHash, "", 99, storageUpdates, 0, bytes4(0), _emptyNonSignerStakesAndSignature());
    }

    function test_verifyAndUpdate_insufficientQuorumReverts() public {
        blsChecker.setStakes(50, 100); // below the 66% threshold
        ...
        a.verifyAndUpdate(msgHash, hex"00", 99, storageUpdates, 0, bytes4(0), _emptyNonSignerStakesAndSignature());
    }
}
